...
module Habitscipline.API.Server.TestUtils where
...
import Habitscipline.API.Server
...
withTestServer func man =
  runNoLoggingT $ withSqlitePool ":memory:" 1 $ \pool -> do
    ...
    liftIO $ do
      ...
      let serverApp = habitsciplineAPIServerApp serverEnv
...
testLogin cenv lf = do
  Headers NoContent (HCons sessionHeader HNil) <- testClientOrErr cenv $ postLogin habitsciplineClient lf
